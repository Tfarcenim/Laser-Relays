...
public class NodeContainer extends Container {
	...
	public NodeBlockEntity nodeBlockEntity;

	public NodeContainer(int windowId, PlayerInventory inv, PacketBuffer data) {
		this(windowId, inv, data.readBlockPos());
	}

	public NodeContainer(int windowId, PlayerInventory inv, BlockPos pos) {
		super(ExampleMod.MENU, windowId);
		World world = inv.player.world;
		nodeBlockEntity = (NodeBlockEntity) world.getTileEntity(pos);
		...
		for (int i = 0; i < 3; i++) {
			for (int j = 0; j < 9; j++) {
				this.addSlot(new Slot(inv, j + i * 9 + 9, j * 18 + playerX, i * 18 + playerY));
			}
		}

		for (int i = 0; i < 9; i++) {
			this.addSlot(new Slot(inv, i, i * 18 + playerX, playerY + 58));
		}
	}
	...
	public ItemStack func_241440_b_(int slotId, int dragType, ClickType clickType, PlayerEntity player) {
		ItemStack itemstack = ItemStack.EMPTY;
				...
				if (!this.dragSlots.isEmpty()) {
					...
					for (Slot slot8 : this.dragSlots) {
						...
						if (slot8 != null && canAddItemToSlot(slot8, mouseStack, true) && slot8.isItemValid(mouseStack) && (this.dragMode == 2 || mouseStack.getCount() >= this.dragSlots.size()) && this.canDragIntoSlot(slot8)) {
							...
							if (slot8 instanceof FilterSlot) {
				...
				if (clickedSlot != null) {
					...
					if (slotStack.isEmpty()) {
						if (!mouseStack.isEmpty() && clickedSlot.isItemValid(mouseStack)) {
							...
							if (clickedSlot instanceof FilterSlot) {
						...
						if (mouseStack.isEmpty()) {
							...
							} else if (clickedSlot instanceof FilterSlot) {
								ItemStack stack = mouseStack.copy();
								stack.setCount(1);
								clickedSlot.putStack(stack);
							}
	...
	public boolean canDragIntoSlot(Slot slotIn) {
		return !(slotIn instanceof FilterSlot);
	}
}
